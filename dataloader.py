...
class dataset(torch_data.Dataset):
    def __init__(self, src, key, tgt):
        self.src = src
        self.key = key
        self.tgt = tgt

    def __getitem__(self, index):
        return self.src[index], self.key[index], self.tgt[index]
...
def padding(data):
    ...
    key_len = [len(k) for k in key]
    key_pad = torch.zeros(len(key), max(key_len)).long()
    ...
    batch['key_len'] = torch.LongTensor(key_len)
